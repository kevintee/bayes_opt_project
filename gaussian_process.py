...
class GaussianCovariance(object):
  ...
  def __init__(self, length_scale, process_variance):
    self.length_scale = length_scale
    self.process_variance = process_variance

  def __str__(self):
    return f'Gaussian({self.length_scale}, {self.process_variance})'
  ...
  def evaluate_kernel_matrix(self, x_centers, x_eval=None):
    ...
    return self.process_variance * numpy.exp(-distance_matrix_squared / self.length_scale ** 2)
...
class CInfinityChebyshevCovariance(object):
  ...
  def __init__(self, length_scale, process_variance, parameter_a=1):
    self.length_scale = length_scale
    self.process_variance = process_variance
    self.parameter_a = parameter_a

  def __str__(self):
    return f'CInfCheb({self.length_scale}, {self.process_variance}, {self.parameter_a})'
  ...
  def evaluate_kernel_matrix(self, x_centers, x_eval=None):
    ...
    return self.process_variance * (1 - a + 2 * a * (1 - b) * numerator / denominator)


# This is only going to work in one dimension -- will consider other stuff eventually, maybe
class GaussianProcess(object):
  def __init__(self, x, y, covariance, mean_function, noise_variance, posterior_regularization=1e-8):
  ...
  def _setup(self):
    ...
    self.kernel_matrix = self.covariance.evaluate_kernel_matrix(self.x)
